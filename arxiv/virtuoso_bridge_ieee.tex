...
This decoupling allows VirtuosoClient to be tested without SSH infrastructure, and changing the transport from SSH to VPN or direct LAN requires no client code changes. The tunnel process is detached and survives across Python script invocations, and port conflicts are resolved automatically by trying successive ports and writing the working port back to the configuration file. The system also supports \textbf{multiple connection profiles}: by suffixing environment variables (e.g.\ \texttt{VB\_REMOTE\_HOST\_gpu1}), users can manage connections to several remote machines simultaneously from a single project, each with independent tunnel state and lifecycle.
...
\paragraph{CLI-first lifecycle.} After the one-time Virtuoso-side setup step, the bridge lifecycle is managed through the commands \texttt{virtuoso-bridge start}, \texttt{status}, \texttt{restart}, \texttt{stop}, and \texttt{license}. Agents invoke these commands without repeated GUI interaction. The \texttt{status} command reports tunnel health, daemon responsiveness, and remote hostname verification. The \texttt{license} command separately checks Spectre license availability via \texttt{lmstat}. All commands accept a \texttt{-{}-profile} flag for multi-machine scenarios.

\paragraph{Skill files.} The \texttt{skills/} directory contains structured documents covering the virtuoso and spectre workflows. Each skill follows a progressive-disclosure design: a concise \texttt{SKILL.md} (always loaded into the agent's context) establishes the mental model---what the system is, how it works, and when to use each abstraction level---before presenting code patterns. Detailed API references, edge-case documentation, and netlist syntax are placed in \texttt{references/} subdirectories and loaded only when the agent needs them. This structure keeps the agent's context lean while ensuring deep reference material is available on demand.
...
The system automatically sources the Cadence environment on the remote host, uploads netlists and include files including Verilog-A modules, executes Spectre, downloads PSF results, and parses them into Python data structures. Multiple simulation modes are supported, including basic Spectre, APS, and Spectre~X. In remote mode, \texttt{SpectreSimulator.from\_env()} automatically reuses the SSH connection established by \texttt{virtuoso-bridge start}, so all file transfers and command execution share the same ControlMaster connection as VirtuosoClient. A \texttt{SpectreSimulator.local()} factory method is also provided for local execution without SSH.

License availability is checked via the separate \texttt{virtuoso-bridge license} command, which queries \texttt{lmstat} on the remote host and reports feature usage. This helps prevent agents from submitting simulations that would wait indefinitely for unavailable licenses.
...
Virtuoso-Bridge does not introduce a new Virtuoso execution mechanism. Instead, it provides a structured encapsulation of the standard SKILL IPC facility through an agent-native tool boundary with decoupled transport and execution layers, unified SSH connection management via ControlMaster multiplexing, three levels of programmability, workflow support for visual verification and simulation, lifecycle management for repeated agent use, and multi-profile support for simultaneous control of multiple remote machines.
